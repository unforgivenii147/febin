...
def download_file(url: str, dest_dir: Path) -> None:
    filename = Path(
        urllib.request.urlparse(url).path).name or "downloaded_file"
...
def process_url(url: str, download_dir: Path | None = None) -> str:
    try:
        ...
        if download_dir and size <= MAX_DOWNLOAD_SIZE:
            user_input = input(
                f"Do you want to download this file (size: {size_str})? (y/n): "
            ).strip().lower()
            if user_input == "y":
            ...
            print(
                "File is too large to download or no download directory specified."
            )
...
def main() -> None:
    ...
    download_dir = Path(args.download) if args.download else Path(
    ...
    if input_path.is_file():
        ...
        updated_lines = [
            process_url(line.strip(), download_dir) for line in lines
            if line.strip()
        ]
