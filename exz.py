...
def extract_with_lzma(xz_path, remove_original=False):
    try:
        xz_path = Path(xz_path)

        if not xz_path.exists():
            return False, xz_path, None, "File does not exist"

        if xz_path.suffix != ".xz":
            return False, xz_path, None, "Not an .xz file"

        output_path = xz_path.with_suffix("")

        if output_path.exists():
            return False, xz_path, output_path, "Output file already exists"

        with lzma.open(xz_path, "rb") as compressed_file:
        ...
        return False, xz_path, None, f"LZMA error: {e}"
    except PermissionError as e:
        return False, xz_path, None, f"Permission denied: {e}"
    except Exception as e:
        return False, xz_path, None, f"Unexpected error: {e}"
...
def extract_with_system_xz(xz_path, remove_original=False):
    try:
        ...
        if result == 0:
            ...
            return False, xz_path, None, f"System xz command failed with code {result}"
  ...
  %(prog)s --system --workers 4 # Use system xz with 4 worker processes
    ...
    try:
        with Pool(processes=args.workers) as pool:
            for i, result in enumerate(pool.imap_unordered(process_file, process_args), 1):
                success, xz_path, output_path, error = result
