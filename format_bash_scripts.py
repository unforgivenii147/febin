        ...
        if item.is_file() and not item.suffix:
            ...
            try:
                with item.open("r", encoding="utf-8") as f:
                    ...
                    if first_line.strip().startswith("#!") and ("bash" in first_line or "/sh" in first_line or "env sh" in first_line):
...
if __name__ == "__main__":
    ...
    if found_scripts:
        print("Found scripts without extension (relative paths):")
        for script_path in found_scripts:
            print(script_path.relative_to(cwd))
            cmd=["shfmt","-w",f"{str(script_path)}"]
            subprocess.run(cmd)
    else:
        print("No scripts without extension found in the current directory or its subdirectories.")
