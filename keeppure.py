    ...
    if os.path.exists(record_file):
        try:
            with open(record_file, 'r') as f:
                reader = csv.reader(f)
                for row in reader:
                    if row and any(row[0].endswith(ext) for ext in ['.so', '.pyd', '.dll', '.dylib']):
                        return pkg_name
        except:
            pass
    return None
...
def clean_requirements_txt(requirements_file='requirements.txt'):
    ...
    # Get binary packages (fast parallel processing)
    ...
    # Separate comments and requirements
