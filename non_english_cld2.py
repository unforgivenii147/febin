...
TEXT_EXTENSIONS = {
    '.txt', '.md', '.rst', '.tex',
    '.py', '.js', '.html', '.htm', '.css', '.php', '.rb', '.go', '.rs',
...
class LanguageDetector:
    ...
    def scan_directory(self, directory, show_progress=True, only_report_non_english=True):
        ...
        for root, dirs, files in os.walk(directory):
            ...
            for file in files:
                ...
                if show_progress:
                    print(f"\r📊 Processing: {filepath} [Files: {self.stats['total_files']}]", end='', flush=True)
    ...
    def report_results(self, only_report_non_english=True):
        ...
        print(f"📋 Summary: {english_files} English, {len(self.stats['non_english'])} non-English, {self.stats['skipped_binary']} binary, {self.stats['skipped_small'] + self.stats['skipped_error']} unanalyzable")
