        ...
        try:
            ...
            for _i, line in enumerate(reader.read_lines(file_path, encoding)):
                ...
                if len(chunk) >= chunk_size:
                    sorted_chunk = self.sort_in_memory(
                        chunk,
                        reverse,
                        case_insensitive,
                    )
...
class FileSorter(LineProcessor):
    ...
    def process_file(
        ...
        try:
            ...
            if sort:
                lines = self.sorter.sort_in_memory(
                    lines,
                    reverse,
                    case_insensitive,
                )
            ...
            return {
                ...
                after,
                "after":
                self.format_size(after),
                "duplicate_lines": (unique_count if unique else 0),
                "size_reduction":
                (original_size - after if original_size > 0 else 0),
                "size_reduction_pct":
                (((original_size - after) / original_size *
