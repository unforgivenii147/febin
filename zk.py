...
def compile_to_bytecode(directory):
    ...
    try:
        subprocess.run(
            [sys.executable, "-m", "compileall", "-b", "-q", directory],
            check=True,
            timeout=60)
        # Remove .py files after compilation
...
def get_site_packages():
    ...
    if not site_packages:
        # Fallback for Termux or non-standard installations
...
def main():
    ...
    # Skip single-file modules
    if is_single_file_module(pkg_path):
        print_warning(
            f"Skipping '{pkg_name}' - single file modules are not supported")
        sys.exit(0)
    # Verify it's a proper package directory
    if not is_package_directory(pkg_path):
        print_warning(f"Skipping '{pkg_name}' - not a multi-file package")
        sys.exit(0)
    site_packages = get_site_packages()
    zip_dir = site_packages  # Save in same directory as requested
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Compile to bytecode if requested
        if use_pyc:
        ...
        if so_files:
            ...
            # C extensions stay in site-packages, don't include in zip
        ...
        compression_ratio = (1 - compressed_size /
