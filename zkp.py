...
def compile_to_bytecode(directory, opt_level=2):
    try:
        ...
        if opt_level == 1:
            ...
            opt_flag = ["-OO"]
        subprocess.run([sys.executable] + opt_flag +
                       ["-m", "compileall", "-b", "-q", directory],
                       check=True,
                       timeout=60)
    ...
    try:
        ...
        if result.returncode == 0 and "OK" in result.stdout:
            ...
            if backup_path and os.path.exists(backup_path):
                ...
                pkg_path = os.path.join(site_packages,
    ...
    with tempfile.TemporaryDirectory() as backup_tmp:
        try:
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                tmp_pkg_path = os.path.join(tmp_dir,
                                            os.path.basename(pkg_path))
                shutil.copytree(pkg_path, tmp_pkg_path)
                if use_pyc:
                    print_warning(
                        f"Compiling to bytecode (optimization level: {opt_level})..."
                    )
                ...
                if so_files:
                    print_warning(
                        "C extensions detected. Keeping in site-packages...")
...
def main():
    ...
    if args.command == "zip":
        success = zip_package(args.package,
                              use_pyc=args.pyc,
